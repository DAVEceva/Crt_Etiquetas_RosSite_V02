...
function handleSearchSubmit() {
    ...
    if (label) {
        // Si ya estaba validada, solo aviso sonoro y muestro info
        if (!label.validado) {
            label.validado = true; // AUTO‑VALIDA
            saveState(); // Guarda en localStorage
            updateFooter(); // Actualiza contador global
            renderListView(); // Refresca vista actual (ruta/ciudad/...)
        }

        playSuccessSound();
        displaySearchResult(label); // Muestra detalle ya marcado como validado
        searchInput.value = '';
    } else {
        playErrorSound();
        searchInput.value = '';
